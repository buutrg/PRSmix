...
combine_PGS = function(
	...
	for (train_size in train_size_list) {
		...
		if (!read_pred_training_1) {
			...
			pred_acc_train_allPGS_summary = get_acc_prslist_optimized(train_df, pgs_list_all, covar_list, liabilityR2, alpha=pval_thres, isbinary)
		...
		bestPRS_acc = eval_prs(test_df, bestPRS, covar_list, liabilityR2, alpha=pval_thres, isbinary)
		...
		if (!read_pred_testing_1) {
			pred_acc_test_trait = get_acc_prslist_optimized(test_df, pgs_list,  covar_list, liabilityR2, alpha=pval_thres, isbinary)
